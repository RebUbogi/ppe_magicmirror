...
Module.register("MMM-ProfileSwitcher", {
	...
	makeNotification: function (messages) {
		if (messages) {
			...
			if (text.length > 0) {
				...
				text = text.length === 1 ? text[0] : text[Math.floor(Math.random() * text.length)];
	...
	set_timer: function (data) {
		...
		this.timer = setTimeout(this.change_profile.bind(this), data.time || this.config.defaultTime, data.profile || this.config.defaultClass);
	},
